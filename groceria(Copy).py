...
try:
    cursor.executemany('INSERT INTO dairy (product_id, exp_date, price, quantity, discount, product_name, description) VALUES (?, ?, ?, ?, ?, ?, ?)', dairy_products)
    cursor.executemany('INSERT INTO veges (product_id, exp_date, price, quantity, discount, product_name, description) VALUES (?, ?, ?, ?, ?, ?, ?)', veges_products)
    cursor.executemany('INSERT INTO fruits (product_id, exp_date, price, quantity, discount, product_name, description) VALUES (?, ?, ?, ?, ?, ?, ?)', fruits_products)
    cursor.executemany('INSERT INTO supplies (product_id, exp_date, price, quantity, discount, product_name, description) VALUES (?, ?, ?, ?, ?, ?, ?)', supplies_products)
...
frame_list=[product_frame,tabs_frame,signup_mainframe,shop_scrollframe1,shop_widgets,shop_widgets_menu,supplies_frame,fruits_frame,dairy_frame,vegetable_frame,search_frame,profile_frame,orders_frame,settings_frame,supports_frame,notification_frame,cart_frame,pro_frame]
...
v001=ctk.CTkImage(Image.open(r'beetroot.png'),size=(100,100))
d001=ctk.CTkImage(Image.open(r'cheese.png'),size=(80,80))
f001=ctk.CTkImage(Image.open(r'pear.png'),size=(80,80))
#s001=ctk.CTkImage(Image.open(r'milk.png'))
...
def signup_user():
    ...
    password_value = password.get().strip()  # Use a different name to avoid overwriting the entry widget
    confirm_password_value = confirm_password.get().strip()

    # Check if any field is empty    
    if not first_name:
        messagebox.showerror("Error", "First name field cannot be empty")
        return
    if not last_name:
        messagebox.showerror("Error", "Last name field cannot be empty")
        return
    if not email:
    ...
    if not confirm_password_value:
        messagebox.showerror("Error", "Confirm Password field cannot be empty")
        return

    # Check if first and last name contain any numbers
    ...
    try:
       ...
       # Create user-specific table for orders
...
def login_user():
    global email_address_login, user_email
    ...
    try:
        ...
        if user:
            ...
            if hashed_password == user[3]:  # Assuming password is the 4th column
                messagebox.showinfo("Success", "Login successful!")
                checked()
                # Here you can add code to switch to the main application window
            else:
...
###   PRODUCT DISPLAY FOR SUPPLIES CATEGORY  ###
    ...
    def show_product_details_supplies(product_id, product_name, price, discount, description, image_path):
    # Hide other frames
  ...
  # Add a back button
        ...
        back_button = ctk.CTkButton(product_frame, text="", image=backbutn, command=back,fg_color='#05161a', hover_color='#05161a', height=55, width=55, corner_radius=5)
        back_button.place(x=40, y=10)

    # Display product name                           
        product_name_label = ctk.CTkLabel(pro_frame, text=product_name, font=('Helvetica', 24), text_color="black", fg_color="white")
        product_name_label.place(x=530, y=50)
    ...
    # Display price and discount
        price_label = ctk.CTkLabel(pro_frame, text=f"Price: ₹{price}", font=('Helvetica', 18), text_color="black")
        price_label.place(x=500, y=150)
        discount_label = ctk.CTkLabel(pro_frame, text=f"Discount: {discount}%", font=('Helvetica', 18), text_color="black")
        discount_label.place(x=500, y=200)

    # Display description
        description_label = ctk.CTkLabel(pro_frame, text=f"Description: {description}", font=('Helvetica', 16), wraplength=500, justify="left", text_color="black")
        description_label.place(x=500, y=250)

    # Add-to-cart button
        add_to_cart_button = ctk.CTkButton(pro_frame, text="Add to Cart", command=lambda: add_to_cart(product_id, product_name, price), fg_color='#0c7075', font=('Helvetica', 18), height=50, width=200)
        add_to_cart_button.place(x=580, y=350)
        
###   PRODUCT DISPLAY FOR FRUITS CATEGORY  ###
    
    def show_product_details_fruits(product_id, product_name, price, discount, description, image_path):
    # Hide other frames
        for frame in frame_list:
            frame.place_forget()
            product_frame.place(x=30, y=30)
...
###   PRODUCT DISPLAY FOR VEGGIES CATEGORY  ###
    ...
    def show_product_details_veggies(product_id, product_name, price, discount, description, image_path):
    # Hide other frames
        for frame in frame_list:
            frame.place_forget()
            product_frame.place(x=30, y=30)
...
###   PRODUCT DISPLAY FOR DAIRY CATEGORY  ###
    ...
    def show_product_details_dairy(product_id, product_name, price, discount, description, image_path):
    # Hide other frames
        for frame in frame_list:
            frame.place_forget()
            product_frame.place(x=30, y=30)
  ...
  # Add a back button
    ...
    # Scrollable frame for displaying products with dynamically adjusted height
        ...
        try:
        # Fetch product details from supplies table
            cursor.execute("SELECT product_id, product_name, price, discount,description FROM supplies")
            products = cursor.fetchall()

        # Define positions limited by the actual number of products
            positions = [(10, 10), (480, 10), (10, 210), (480, 210), (10, 410), (480, 410),
                     (10, 610), (480, 610), (10, 810), (480, 810)][:len(products)]
        
        # Set dynamic height for scroll frame based on number of products
        ...
        # Display each product
            for i, product in enumerate(products):
                product_id, product_name, price, discount,description= product
    ...
    # Scrollable frame for displaying products with dynamically adjusted height
        ...
        try:
        # Fetch product details from supplies table
            cursor.execute("SELECT product_id, product_name, price, discount,description FROM fruits")
            products = cursor.fetchall()
        ...
        # Display each product
            ...
            # Product Image
                try:
                    ...
                    img = Image.open(image_path)
                    ctk_image = ctk.CTkImage(img, size=(100, 100))
                    image_button = ctk.CTkButton(product_label, text='', image=ctk_image, height=140, width=140, corner_radius=20, fg_color='white',border_width=3,bg_color='white',hover_color='white',command=lambda p_id=product_id, p_name=product_name, p_price=price, p_discount=discount, p_desc=description: show_product_details_fruits(p_id, p_name, p_price, p_discount, p_desc, f"images/{p_id}.png"))
                    image_button.place(x=20, y=20)
                except FileNotFoundError:
                    print("Image file not found:", image_path)
                # Default display if image is not found
    ...
    # Scrollable frame for displaying products with dynamically adjusted height
        ...
        try:
        # Fetch product details from supplies table
            cursor.execute("SELECT product_id, product_name, price, discount,description FROM dairy")
            products = cursor.fetchall()
        ...
        # Display each product
            ...
            # Product Image
                try:
                    ...
                    img = Image.open(image_path)
                    ctk_image = ctk.CTkImage(img, size=(100, 100))
                    image_button = ctk.CTkButton(product_label, text='', image=ctk_image, height=140, width=140, corner_radius=20, fg_color='white',border_width=3,bg_color='white',hover_color='white',command=lambda p_id=product_id, p_name=product_name, p_price=price, p_discount=discount, p_desc=description: show_product_details_dairy(p_id, p_name, p_price, p_discount, p_desc, f"images/{p_id}.png"))
                    image_button.place(x=20, y=20)
                except FileNotFoundError:
                    print("Image file not found:", image_path)
                # Default display if image is not found
    ...
    # Scrollable frame for displaying products with dynamically adjusted height
        ...
        try:
        # Fetch product details from supplies table
            cursor.execute("SELECT product_id, product_name, price, discount,description FROM veges")
            products = cursor.fetchall()
        ...
        # Display each product
            ...
            # Product Image
                try:
                    ...
                    img = Image.open(image_path)
                    ctk_image = ctk.CTkImage(img, size=(100, 100))
                    image_button = ctk.CTkButton(product_label, text='', image=ctk_image,height=140, width=140, corner_radius=20, fg_color='white',border_width=3,bg_color='white',hover_color='white',command=lambda p_id=product_id, p_name=product_name, p_price=price, p_discount=discount, p_desc=description: show_product_details_veggies(p_id, p_name, p_price, p_discount, p_desc, f"images/{p_id}.png"))
                    image_button.place(x=20, y=20)
                except FileNotFoundError:
                    print("Image file not found:", image_path)
                # Default display if image is not found
...
#CATEGORY TABS
    ...
    images = [produce1, produce2, produce3, produce4]
    prices = ["₹500", "₹600", "₹750", "₹250"]
    
    for index, (product_label, img, price) in enumerate(zip(product_labels, images, prices)):
        image_button = ctk.CTkButton(product_label, text='', height=90, width=90, corner_radius=20, fg_color='white', font=('Helvetica', 50), border_width=3, image=img)
        image_button.place(x=10, y=10)  # Positioning inside product label
...
## NESTED SEARCH BOX ##
    ...
    def search_box(nested_searchbox, search_frame, user_email):
    ...
    # Connect to the database
        ...
        try:
            ...
            if found_products:
                ...
                # Display details
                    details_text = f"{product_name}\n\n Price: ₹{price}\n{discount}% Off\nExpiry Date -  {exp_date}\nQuantity{quantity} \n {description}"
                    details_label = ctk.CTkLabel(search_frame, text=details_text, anchor="w", font=("Helvetica", 22), width=300, height=100,fg_color='white')
                    details_label.place(x=500, y=150)

                # Add to Cart button
                    add_to_cart_button = ctk.CTkButton(search_frame, text="Add to Cart",bg_color='white',fg_color='#0c7075',width=100,height=60,corner_radius=20, font=("Helvetica", 28),command=lambda p=product: add_to_cart(product, user_email))
                    add_to_cart_button.place(x=580, y=380)
            else:
    ...
    def add_to_cart(product, user_email):
        product_id, exp_date, price, quantity, discount, product_name, description = product
        ...
        sanitized_table_name = user_email.replace('@', '_at_').replace('.', '_dot_')
    
        try:
            cursor.execute(f"INSERT INTO {sanitized_table_name} (order_id, product_name, quantity, price) VALUES (?, ?, ?, ?)",
                       (order_id, product_name, quantity, price))
            conn.commit()
            messagebox.showinfo("Success", f"Added {product_name} to cart with Order ID: {order_id}")
        except sqlite3.Error as e:
            messagebox.showerror("Database Error", f"An error occurred: {e}")
        finally:
            conn.close()
...
#SEARCH TAB
    def search_tab(searchbox, search_frame, user_email):
        ...
        try:
            ...
            if found_products:
                ...
                # Display product information
                    ...
                    search_button=ctk.CTkButton(nested_searchbox,text='',image=seac,fg_color='#05161a',width=50,height=30,corner_radius=20,border_width=3,bg_color='#ffffff',border_color='#0f969c',command=lambda: search_box(nested_searchbox, search_frame, user_email))
...
#CART TAB      
    ...
    search_button=ctk.CTkButton(shop_widgets,text='',image=seac,fg_color='#05161a',width=50,height=30,corner_radius=20,border_width=3,bg_color='#ffffff',border_color='#0f969c',command=lambda: search_tab(searchbox, search_frame, user_email))    
